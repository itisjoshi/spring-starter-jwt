package com.beanhq.utils;

/**
 * Created by Prathap Manohar Joshi
 */
public class Constants {
	
	public static final String BASE_PACKAGE = "com.beanhq";  
	
	public static final String DISPATCHER = "dispatcher";  
	
	public static final String SLASH = "/api/*";  
	
	public static final String NEW_LINE = "\n";  
	
	public static final String INVALID_FIELD_ERROR_MESSAGE = "Request is not submitted due to invalid field.";  
	
	public static final String RESOURCE_NOT_FOUND_ERROR_MESSAGE = "Resource not found.";  
	
	public static final String SERVICE_UNAVAILABLE_ERROR_MESSAGE = "Service Unavailable. Unknown exception occurred."; 
	
}